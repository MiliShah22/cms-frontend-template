      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-10 py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-xs leading-6 text-[#94a3b8] mb-4">
          ...
          {[
            { title:"Shop", links:[
              {label:"New Arrivals",href:"/new-arrivals"},{label:"Collections",href:"/collections"},
              {label:"Brands",href:"/brands"},{label:"Offers",href:"/offers"},{label:"Wishlist",href:"/wishlist"},
            ]},
            { title:"Help", links:[
              {label:"FAQ",href:"#"},{label:"Shipping Policy",href:"#"},
              {label:"Returns & Refunds",href:"#"},{label:"Track Order",href:"#"},{label:"Contact Us",href:"#"},
            ]},
          ].map((col)=>(
            <div key={col.title}>
              <p className="text-xs font-bold tracking-widest uppercase text-[#6366f1] mb-4">{col.title}</p>
              <div className="flex flex-col gap-2.5">
                {col.links.map((l)=>(
                  <Link key={l.label} href={l.href} className="text-xs text-[#94a3b8] hover:text-white transition-colors">{l.label}</Link>
                ))}
              </div>
            </div>
          ))}
